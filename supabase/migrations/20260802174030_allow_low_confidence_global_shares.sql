-- Global sharing is an explicit user-consented publication path. A model
-- confidence score is retained as evidence, but it is not a publication gate.
-- The API still requires a finite score in the inclusive [0, 1] range.

alter table public.scan_contributions
  drop constraint if exists scan_contributions_confidence_check;
alter table public.scan_contributions
  add constraint scan_contributions_confidence_check
  check (confidence >= 0 and confidence <= 1);

alter table public.global_share_intents
  drop constraint if exists global_share_intents_confidence_check;
alter table public.global_share_intents
  add constraint global_share_intents_confidence_check
  check (confidence >= 0 and confidence <= 1);

-- Mobile JSON carries a floating-point value while the tables store four
-- decimal places. Treat values within one half of the stored precision as the
-- same score, and publish the canonical score from the reserved intent.
...
begin
  ...
  insert into public.scan_contributions (
    ...
    intent.source, intent.model_version, intent.photo_path, 'published'
