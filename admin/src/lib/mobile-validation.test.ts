...
describe("global-share API validation", () => {
  ...
  it("accepts a low but valid confidence and canonical metadata", () => {
  ...
  it.each([
    ["negative confidence", { ...validIntent, confidence: -0.0001 }],
  ...
  it("canonicalizes confidence to the database precision without applying a minimum", () => {
    const result = validateShareIntent({ ...validIntent, confidence: 0.034567 });

    expect(result).toEqual({
      ok: true,
      value: { ...validIntent, confidence: 0.0346 },
    });
  });
  ...
  it("canonicalizes low-confidence completion metadata to the database precision", () => {
    const path = globalSharePath(USER_ID, SCAN_ID, SHA256);
    expect(
      validateShareCompletion({ ...validIntent, confidence: 0.034567, path }, USER_ID),
    ).toEqual({
      ok: true,
      value: {
        clientScanId: SCAN_ID,
