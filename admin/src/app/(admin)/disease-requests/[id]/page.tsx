...
export default async function RequestReview({
  ...
  searchParams: Promise<{ reviewed?: string; outcome?: string }>;
}) {
  ...
  const { data: request, error: requestError } = await supabase
    ...
    .select("*,disease_request_photos(position,object_path)")
    .eq("id", id)
    .maybeSingle();
  if (requestError) throw new Error("The disease request could not be loaded.");
  if (!request) notFound();

  const photos = ([...(request.disease_request_photos ?? [])] as RequestPhoto[]).sort(
    (a, b) => a.position - b.position,
  );
  const signedResult = photos.length
    ? await supabase.storage.from("eggplant-scans").createSignedUrls(
        photos.map((photo) => photo.object_path),
        300,
      )
    : { data: [], error: null };
  ...
  const reviewedStatus = query.reviewed;
  const reviewed = reviewedStatus ? requestStatusLabel(reviewedStatus) : "";
  const reviewSucceeded = Boolean(
    reviewedStatus &&
      REQUEST_REVIEW_STATUSES.includes(
        reviewedStatus as (typeof REQUEST_REVIEW_STATUSES)[number],
      ),
  );
  ...
  return (
    <div className="fade-up mx-auto max-w-6xl">
      ...
      {reviewSucceeded && (
        <p
          role="status"
          className="status-banner mt-4 flex items-center gap-2 rounded-xl border border-[#bfe4c5] bg-[#f1fbf2] p-3 text-sm font-semibold text-[#247936]"
        >
          <CheckCircle2 size={17} />
          {query.outcome === "unchanged"
            ? "No request changes were needed."
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1.4fr)_minmax(19rem,.8fr)]">
        <section className="min-w-0">
          ...
          <section className="surface mt-5 p-5">
            <p className="text-xs font-semibold uppercase tracking-[.12em] text-[#278b3d]">
              Private disease request
            </p>
            ...
            <dl className="mt-4 text-sm">
              <div className="flex flex-wrap justify-between gap-3 border-t py-3">
                <dt>Status</dt>
                <dd className="font-semibold">{requestStatusLabel(request.status)}</dd>
              </div>
              <div className="flex flex-wrap justify-between gap-3 border-t py-3">
                <dt>Model version</dt>
                <dd className="break-all font-mono text-xs">{request.model_version}</dd>
              </div>
            </dl>
          </section>
        </section>
        <aside className="min-w-0 space-y-5">
          <form action={updateDiseaseRequest} className="surface p-5">
