...
export default async function GlobalScansPage({ searchParams }: { searchParams: Promise<{ page?: string; status?: string; q?: string }> }) {
  ...
  return <div className="fade-up mx-auto max-w-[1240px]">
      <header className="flex flex-wrap items-end justify-between gap-4"><div><h1 className="text-3xl font-bold tracking-[-.03em]">Global scans</h1><p className="mt-1 max-w-2xl text-sm text-[#6f6b80]">Review shared community photos. Scans marked hidden for review are not shown to mobile users.</p></div><p className="rounded-xl border border-[#e5ddec] bg-[#fffdfd] px-3 py-2 text-sm font-semibold text-[#5e596e]">{count ?? 0} matching scan{count === 1 ? "" : "s"}</p></header>
      <form method="get" className="surface mt-6 grid gap-3 p-4 sm:grid-cols-[minmax(12rem,.55fr)_minmax(14rem,1fr)_auto] sm:items-end"><label className="grid gap-1.5 text-sm font-semibold">Status<select name="status" defaultValue={status} className="focus-ring h-11 rounded-xl border border-[#e5ddec] bg-[#fffdfd] px-3"><option value="published">Published</option><option value="all">All statuses</option><option value="quarantined">Hidden for review</option><option value="removed">Removed</option></select></label><label className="grid gap-1.5 text-sm font-semibold">Disease search<div className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[#777286]" size={17}/><input name="q" defaultValue={search} maxLength={80} placeholder="e.g. melon thrips" className="focus-ring h-11 w-full rounded-xl border border-[#e5ddec] bg-[#fffdfd] py-2 pl-10 pr-3 font-normal"/></div></label><div className="flex flex-wrap items-center gap-2"><button type="submit" aria-label="Apply scan filters" className="focus-ring action-button inline-flex h-11 items-center gap-2 rounded-xl bg-[#5b3295] px-4 font-semibold text-white"><Filter size={16}/>Apply filters</button>{(status !== "published" || search) && <Link href="/global-scans" className="focus-ring inline-flex h-11 items-center gap-2 rounded-xl border border-[#dcd8e4] px-3 text-sm font-semibold text-[#5f596d]"><X size={16}/>Clear</Link>}</div></form>
      <p className="mt-3 flex flex-wrap items-center gap-2 text-xs text-[#68687c]" aria-live="polite"><span className="font-semibold text-[#3d2269]">Showing</span><span className="rounded-full bg-[#f1ecf8] px-2.5 py-1 font-semibold text-[#5b3295]">{status === "published" ? "Published" : status === "all" ? "All statuses" : scanStatusLabel(status)}</span>{search && <span className="rounded-full bg-[#f1ecf8] px-2.5 py-1 font-semibold text-[#5b3295]">Disease: {search}</span>}</p>
