...
export default async function GlobalScanDetail({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ moderated?: string; outcome?: string; page?: string; status?: string; q?: string }> }) {
  ...
  return <div className="fade-up mx-auto max-w-[1240px]">
    <header className="flex flex-wrap items-center justify-between gap-3"><Link href="/global-scans" className="inline-flex min-h-10 items-center gap-2 text-sm font-semibold text-[#1f6b3a]"><ArrowLeft size={17}/>Back to scans</Link><nav aria-label="Scan details" className="flex flex-wrap gap-2"><Link aria-label="Previous scan" aria-disabled={!previousId} tabIndex={previousId ? undefined : -1} href={previousId ? detailHref(previousId) : "#"} className={`focus-ring inline-flex min-h-10 items-center gap-1 rounded-xl border border-[#d5e2d3] px-3 text-sm font-semibold ${previousId ? "" : "pointer-events-none opacity-45"}`}><ChevronLeft size={17}/>Previous</Link><Link aria-label="Next scan" aria-disabled={!nextId} tabIndex={nextId ? undefined : -1} href={nextId ? detailHref(nextId) : "#"} className={`focus-ring inline-flex min-h-10 items-center gap-1 rounded-xl border border-[#d5e2d3] px-3 text-sm font-semibold ${nextId ? "" : "pointer-events-none opacity-45"}`}>Next<ChevronRight size={17}/></Link></nav></header>
    ...
    <div className="mt-4 grid gap-5 xl:grid-cols-[minmax(0,1.45fr)_minmax(19rem,.8fr)]">
      <div className="min-w-0 space-y-5"><section className="surface overflow-hidden"><GlobalScanImageToggle originalUrl={signed?.signedUrl ?? null} annotatedUrl={annotatedSigned?.signedUrl ?? null} alt="Shared eggplant disease photo" /></section><section className="surface p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-lg font-bold">Reports about this scan</h2><p className="mt-1 text-sm text-[#68766b]">These are user flags. Check the photo before hiding or restoring the scan.</p></div><span className="rounded-full bg-[#fff0dd] px-2.5 py-1 text-xs font-bold text-[#995a06]">{reports.length}</span></div>{reports.length === 0 ? <p className="mt-4 rounded-xl bg-[#f9fcf7] p-4 text-sm text-[#647166]">No one has reported this scan.</p> : <div className="mt-4 divide-y divide-[#e5ece2]">{reports.map((report) => <article className="py-4 first:pt-0 last:pb-0" key={report.id}><div className="flex flex-wrap items-center justify-between gap-3"><span className="rounded-full bg-[#eaf4e8] px-2.5 py-1 text-xs font-semibold text-[#1f6b3a]">{reportReasonLabel(report.reason)}</span><time className="font-mono text-xs text-[#68766b]">{new Date(report.created_at).toLocaleString()}</time></div>{report.details && <p className="safe-long-content mt-3 whitespace-pre-wrap text-sm leading-6 text-[#5e6d61]">{report.details}</p>}</article>)}</div>}</section></div>
      <aside className="min-w-0 space-y-5"><section className="surface p-5"><h1 className="safe-long-content text-2xl font-bold capitalize">{scan.disease_id.replaceAll("-", " ")}</h1><div className="mt-3"><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${scanStatusTone(scan.status)}`}>{scanStatusLabel(scan.status)}</span></div><dl className="mt-4 divide-y divide-[#e5ece2] text-sm"><Row label="Confidence" value={`${Math.round(Number(scan.confidence) * 100)}%`}/><Row label="Source" value={scan.source}/><Row label="Reports" value={String(reports.length)}/><Row label="Model" value={scan.model_version}/></dl></section><section className="surface p-5"><h2 className="font-bold">Scan actions</h2><p className="mt-1 text-xs leading-5 text-[#68766b]">Hide a scan from the public while you check it, or publish it again after review.</p><div className="mt-4 grid gap-2"><Action id={id} status="published" page={page} listStatus={requestedStatus} search={search} label="Publish / restore" icon={<CheckCircle2 size={17}/>} style="border-[#70b77b] text-[#247936]"/><Action id={id} status="quarantined" page={page} listStatus={requestedStatus} search={search} label="Hide from public" icon={<ShieldAlert size={17}/>} style="border-[#e0a44e] text-[#915707]"/><Action id={id} status="removed" page={page} listStatus={requestedStatus} search={search} label="Remove permanently" icon={<Trash2 size={17}/>} style="border-[#e38b96] text-[#b12d40]"/></div></section></aside>
    </div>
  </div>;
}

function Row({ label, value }: { label: string; value: string }) {
  return <div className="flex flex-wrap justify-between gap-4 py-3"><dt className="text-[#647166]">{label}</dt><dd className="safe-long-content max-w-[65%] text-right font-mono font-semibold">{value}</dd></div>;
}
