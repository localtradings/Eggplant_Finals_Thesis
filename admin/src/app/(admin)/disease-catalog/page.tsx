...
export default async function CatalogPage({ searchParams }: { searchParams: Promise<{ published?: string; deleted?: string; outcome?: string; cleanup?: string }> }) {
  ...
  return (
    <div className="fade-up mx-auto max-w-[1240px]">
      ...
      {published && <p role="status" className="status-banner mt-5 flex items-center gap-2 rounded-xl border border-[#bfe4c5] bg-[#f1fbf2] p-3 text-sm font-semibold text-[#247936]"><CheckCircle2 size={17}/>{publishedName || "Disease content"} {params.outcome === "unchanged" ? "was already published." : "published successfully."}</p>}
      ...
      {error ? <p role="alert" className="mt-6 rounded-xl bg-[#fff0f2] p-4 text-sm text-[#a92f40]">The disease catalog is temporarily unavailable.</p> : <section className="mt-6 grid gap-4 lg:grid-cols-2" aria-label="Disease and model class catalog">
        {catalogEntries.map((entry) => {
          ...
          return <Link href={`/disease-catalog/${disease.id}`} key={disease.id} className="surface motion-card p-5"><div className="flex items-start gap-4">{artworkUrl && <div className="relative h-20 w-24 shrink-0 overflow-hidden rounded-xl border border-[#d5e2d3] bg-[#f2f7ef]"><Image src={artworkUrl} alt="" fill sizes="6rem" unoptimized className="object-contain p-1" /></div>}<div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[.12em] text-[#278b3d]">{disease.category.replaceAll("_", " ")}</p><h2 className="mt-1 text-xl font-bold">{en?.name ?? disease.id}</h2></div><span className={`shrink-0 rounded-lg px-2 py-1 font-mono text-xs ${disease.detector_supported === false ? "bg-[#edf7ee] text-[#247936]" : "bg-[#eaf4e8] text-[#1f6b3a]"}`}>{disease.detector_supported === false ? "Library only" : `Class ${disease.model_class_index}`}</span></div><p className="mt-3 text-sm leading-6 text-[#5e6d61]">{en?.symptom_preview || "Content will be published from the bundled offline catalog."}</p></div></div><div className="mt-4 flex flex-wrap gap-2">{disease.disease_references?.filter((reference) => reference.language_tag === "en").map((reference) => <span className="inline-flex items-center gap-1 text-xs font-semibold text-[#1f6b3a]" key={reference.url}>{reference.publisher}<ExternalLink size={12}/></span>)}</div></Link>;
        })}
      </section>}
