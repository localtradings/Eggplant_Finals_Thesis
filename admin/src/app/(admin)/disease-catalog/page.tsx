    ...
    <div className="admin-page catalog-page fade-up mx-auto max-w-[1240px]">
      ...
      {error ? <p role="alert" className="mt-6 rounded-xl bg-[#fff0f2] p-4 text-sm text-[#a92f40]">The disease catalog is temporarily unavailable.</p> : <section className="catalog-grid mt-6" aria-label="Disease and model class catalog">
        {catalogEntries.map((entry) => {
          ...
          return <Link href={`/disease-catalog/${disease.id}`} key={disease.id} className="catalog-card surface motion-card overflow-hidden"><div className="catalog-artwork">{artworkUrl ? <Image src={artworkUrl} alt="" fill sizes="(min-width: 1024px) 30vw, (min-width: 640px) 45vw, 100vw" unoptimized /> : <div className="grid h-full place-items-center text-xs font-semibold uppercase tracking-[.12em] text-[#7d8d77]">Artwork pending</div>}</div><div className="catalog-card-body"><div className="catalog-meta"><p className="text-xs font-semibold uppercase tracking-[.12em] text-[#278b3d]">{disease.category.replaceAll("_", " ")}</p><span className={`shrink-0 rounded-lg px-2 py-1 font-mono text-xs ${disease.detector_supported === false ? "bg-[#edf7ee] text-[#247936]" : "bg-[#eaf4e8] text-[#1f6b3a]"}`}>{disease.detector_supported === false ? "Library only" : `Class ${disease.model_class_index}`}</span></div><h2 className="mt-3 font-bold">{en?.name ?? disease.id}</h2><p className="mt-3 text-sm leading-6 text-[#5e6d61]">{en?.symptom_preview || "Content will be published from the bundled offline catalog."}</p><div className="mt-4 flex flex-wrap gap-2">{disease.disease_references?.filter((reference) => reference.language_tag === "en").map((reference) => <span className="inline-flex items-center gap-1 text-xs font-semibold text-[#1f6b3a]" key={reference.url}>{reference.publisher}<ExternalLink size={12}/></span>)}</div></div></Link>;
        })}
      </section>}
